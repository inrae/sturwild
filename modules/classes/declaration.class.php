...
class Declaration extends ObjetBDD {
	...
	function duplicate($id) {
		...
		if (is_numeric($id) && $id > 0) {
			...
			if ($data["declaration_id"] > 0) {
				...
				$dataNew = array ("declaration_id"=>0,
						"annee"=>$data["annee"],
	...
	function search(array $param) {
		...
		$data = $this->getListeParam ( $sql . $this->fromSearch.$where . $order );
		/*
	...
	function getWhere($param) {
		...
		if (is_numeric ( $param ["annee_debut"] ) && is_numeric ( $param ["annee_fin"] )) {
			$where = " where annee between " . $param ["annee_debut"] . " and " . $param ["annee_fin"];
		} else {
			$where = " where 1 = 1 ";
		}
		if ($param ["statut_id"] > 0 && is_numeric($param["statut_id"])) {
			$where .= " and statut_id = " . $param ["statut_id"];
		}
		if ($param ["ciem_id"] > 0 && is_numeric($param["ciem_id"])) {
			$where .= " and ciem_id = " . $param ["ciem_id"];
		}
		if ($param ["pays_id"] > 0 && is_numeric($param["pays_id"])) {
			$where .= " and pays_id = " . $param ["pays_id"];
		}
		...
		if ($param ["milieu_id"] > 0 && is_numeric($param["milieu_id"])) {
			$where .= " and milieu_id = " . $param ["milieu_id"];
		}
		if ($param ["capture_etat_id"] > 0 && is_numeric($param["capture_etat_id"])) {
			$where .= " and capture_etat_id = " . $param ["capture_etat_id"];
		}
		...
		$where .= " and engin_type_id = " . $param ["engin_type_id"];
		}
		if (strlen ( $param ["libelle"] ) > 0) {
			$where .= " and (upper(pecheur_code) like upper('%" . $param ["libelle"] . "%')";
			$where .= " or upper(interlocuteur) like upper('%" . $param ["libelle"] . "%')";
			if (is_numeric ( $param ["libelle"] ))
				$where .= " or declaration_id = " . $param ["libelle"];
			$where .= ")";
		}
		return $where;
	}
	...
	function getIdFromParam($param) {
		...
		$where = $this->getWhere ( $param );
		return $this->getListeParam($sql.$this->fromSearch.$where);
	}
